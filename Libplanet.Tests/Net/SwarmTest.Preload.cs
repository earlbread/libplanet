...
namespace Libplanet.Tests.Net
{
    public partial class SwarmTest
    {
        ...
        {
            var policy = new BlockPolicy<DumbAction>(new MinerReward(1));
            var mptFx = new DefaultStoreFixture(mpt: true);
            var receiverChain = TestUtils.MakeBlockChain(
                policy, mptFx.Store, stateStore: mptFx.StateStore);
            Swarm<DumbAction> receiverSwarm = CreateSwarm(receiverChain);
            Swarm<DumbAction> seedSwarm = CreateSwarm();
            ...
            try
            {
                await seedSwarm.BlockChain.MineBlock(seedSwarm.Address);
